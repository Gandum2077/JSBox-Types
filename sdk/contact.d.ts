...
interface Contact {
  ...
  pick(
    options?: Omit<ContactTypes.ContactPickOptions, "handler">
  ): Promise<ContactTypes.ContactItem>;
  pick(options: ContactTypes.ContactMultiPickOptions): void;
  pick(
    options?: Omit<ContactTypes.ContactMultiPickOptions, "handler">
  ): Promise<ContactTypes.ContactItem[]>;
  fetch(options: ContactTypes.ContactFetchOptions): void; // 此API实际上是不可用的
  create(options: ContactTypes.ContactCreateOptions): void;
  create(
    options: Omit<ContactTypes.ContactCreateOptions, "handler">
  ): Promise<void>;
  save(options: ContactTypes.ContactSaveOptions): void;
  save(
    options: Omit<ContactTypes.ContactSaveOptions, "handler">
  ): Promise<void>;
  delete(options: ContactTypes.ContactDeleteOptions): void;
  delete(
    options: Omit<ContactTypes.ContactDeleteOptions, "handler">
  ): Promise<void>;
  ...
  addToGroup(options: {
    contact: ContactTypes.ContactItem;
    group: ContactTypes.Group;
  }): void;
