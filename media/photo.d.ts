...
interface Photo {
  ...
  take(
    options: Omit<PhotoTypes.TakeOptions, "handler">
  ): Promise<PhotoTypes.PhotoResponse>;
  pick(options: PhotoTypes.PickOptions): void;
  pick(
    options: Omit<PhotoTypes.PickOptions, "handler">
  ): Promise<PhotoTypes.PhotoResponse>;
  pick(options: PhotoTypes.MultiPickOptions): void;
  pick(
    options: Omit<PhotoTypes.MultiPickOptions, "handler">
  ): Promise<PhotoTypes.MultiPhotosResponse>;
  prompt(options: PhotoTypes.PromptOptions): void; // 仅支持handler形式的回调
  scan(options: PhotoTypes.ScanOptions): void;
  // 仅支持handler形式的回调（与文档描述不符）
  save(options: PhotoTypes.SaveOptions): void; // 仅支持handler形式的回调
  fetch(options: PhotoTypes.FetchOptions): void;
  fetch(
    options: Omit<PhotoTypes.FetchOptions, "handler">
  ): Promise<UIImage[] | NSData[]>;
