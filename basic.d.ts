...
interface MASViewConstraint extends JBBasicValue {
  ...
  equalTo: (
    target: number | MASViewConstraint | MASViewAttribute | AllUIView
  ) => MASViewConstraint;
  lessThanOrEqualTo: (
    target: number | MASViewConstraint | MASViewAttribute | AllUIView
  ) => MASViewConstraint;
...
interface UIBaseView extends JBBasicValue {
  ...
  relayout(): void;
  updateLayout(
    callback: (make: MASConstraintMaker, view: AllUIView) => void
  ): void;
  remakeLayout(
    callback: (make: MASConstraintMaker, view: AllUIView) => void
  ): void;
...
interface UIScrollView extends UIBaseView {
  ...
  // 键盘收起模式: 0: none, 1: on-drag, 2: interactive
...
interface BBCanvasContext extends JBBasicValue {
  ...
  // 添加一条弧线，以 (x, y) 为中点，radius 为半径，startAngle 为起始弧度，
  // endAngle 为终止弧度，clockwise 表示是否顺时针。
  addArcToPoint(
    x1: number,
    y1: number,
    x2: number,
  ...
  drawText(
    rect: JBRect,
    text: string,
    attributes: { color?: UIColor; font?: UIFont }
  ): void; // 将文字绘制到 rect 这个矩形上
}
